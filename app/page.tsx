  ...
  return (
    <main className="shell">
      <aside className={"sidebar " + (menuOpen ? "sidebar-open" : "")}>
        ...
        <div className="sidebar-bottom"><div className="help-card"><div className="help-icon"><CircleHelp size={18}/></div><b>Need a hand?</b><p>Check the fleet operations guide.</p><button onClick={()=>alert("Operations guide is not connected yet.")}>Open help center <ArrowUpRight size={14}/></button></div><div className="profile"><div className="profile-avatar">SB</div><div className="profile-copy"><b>Staff session</b><small>{snapshot?.authenticated ? "Authenticated" : "Not signed in"}</small></div><Settings size={17}/></div></div>
      </aside>
      ...
      <section className="main-area">
        ...
        <div className="content">
          <div className="page-heading"><div><div className="eyebrow"><span className="eyebrow-dot"/> {new Date().toLocaleDateString(undefined,{weekday:"long",month:"long",day:"numeric",year:"numeric"}).toUpperCase()}</div><h1>Fleet overview <span>✦</span></h1><p>{loading ? "Loading your authorized college data…" : "Operational data from your Supabase workspace."}</p></div><button className="date-btn"><Clock3 size={16}/> Today <ChevronDown size={15}/></button></div>
          ...
          : active === "Routes" ? <section className="panel" style={{padding:20}}><div className="panel-head"><div><div className="panel-title">Routes</div><p>Routes available under your college's existing row-level security.</p></div><span className="count-badge">{snapshot?.routes.length??0}</span></div><div className="activity-list">{(snapshot?.routes??[]).map(r=><div className="activity-row" key={r.id}><div className="activity-icon"><Route size={16}/></div><div className="activity-copy"><b>{r.name}</b><span>{r.status||"Status not set"}</span></div></div>)}{!loading&&!(snapshot?.routes.length)&&<div className="empty">No route records returned for this account.</div>}</div></section>
